//Declare variables
    ...
    //Declare forecast function for the next five days 
    function forescast() {
        ...
        $.ajax({
            ...
            var lastdate = Number(moment().format("DD")); 
            var g = 0;
            for (var i = 0; i < response.list.length; i++) {
                ...
                var date = dateTime.split(" ")[0];
                    ...
                    if (lastdate < Number(day)) {
                    $("#day" + g).children(".date").text(month + "/" + day + "/" + year);
                    $("#day" + g).children(".img").attr("src", "https://api.openweathermap.org/img/w/" + response.list[i].weather[0].icon.replace("n", "d") + ".png");
                    $("#day" + g).children(".temp").text("Temp: " + ((response.list[i].main.temp - 273.15) * (9 / 5) + 32).toFixed(2) + "°F");
                    $("#day" + g).children(".hum").text("Humidity: " + response.list[i].main.humidity + "%");
                    lastdate = Number(day);
                    g++;
                }
            }
    ...
    // Declare function to display and save the city searched
